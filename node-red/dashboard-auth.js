...
var token = "123456"; //Password for Dashboard authentication
var htmlForm = '<form >Username: <input name="username" type="text"><br>Password: <input name="token"  type="text"><br><input type="submit" value="Submit"></form>';

function dashboardAuth(req, res, next) {
  var req_username = req.query.username;
  var req_token = req.query.token;
  ...
  if(req_username == undefined || req_token == undefined){
    res.writeHeader(200, {"Content-Type": "text/html"})
    res.write(htmlForm);
    return res.end();	  
  }
  
  if(req_username == username && req_token == token){
	  return next();
  }else{
