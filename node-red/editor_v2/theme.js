/* */
/* Copyright 2016,2020 JS Foundation and other contributors, https://js.foundation/ */
/* Copyright 2016 IBM Corp.                                                 */
/* Copyright 2015 Andrei Tatar                                              */
/* */
/* Licensed under the Apache License, Version 2.0 (the "License");          */
/* you may not use this file except in compliance with the License.         */
/* You may obtain a copy of the License at                                  */
/* */
/* http://www.apache.org/licenses/LICENSE-2.0                               */
/* */
/* Unless required by applicable law or agreed to in writing, software      */
/* distributed under the License is distributed on an "AS IS" BASIS,        */
/* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied. */
/* See the License for the specific language governing permissions and      */
/* limitations under the License. */
/* */
var myTheme = `
<style id='mywystheme'>
@media (max-width: 959px) {

  .md-sidenav-left,
  md-sidenav {
    top: 48px !important;
    z-index: 99;
  }
}

@media (max-width: 959px) and (min-width: 0) and (orientation: portrait) {
  md-toolbar {
    min-height: 48px;
    height: 48px;
    max-height: 48px;
  }
}

@media (min-width: 960px) {

  .md-sidenav-left,
  md-sidenav {
    top: 64px;
    z-index: 99;
  }
}

@media (max-width: 660px) {

  md-sidenav,
  md-sidenav.md-locked-open,
  md-sidenav.md-closed.md-locked-open-add-active {
    min-width: 64px !important;
    width: auto;
    max-width: 200px !important;
  }
}

@media (min-width: 661px) {

  md-sidenav,
  md-sidenav.md-locked-open,
  md-sidenav.md-closed.md-locked-open-add-active {
    min-width: 64px !important;
    width: auto;
    max-width: 320px !important;
  }
}

md-sidenav ng-md-icon {
  padding-right: 4px;
}

md-sidenav i {
  padding-right: 4px;
}

md-sidenav md-list-item .md-list-item-inner>p {
  line-height: 20px;
}

md-toast .md-toast-content {
  height: auto;
}

md-toast .md-toast-error {
  color: rgba(255, 64, 41, 0.84);
  font-weight: 500;
  font-size: large;
}

md-backdrop.md-sidenav-backdrop {
  z-index: 80;
}

md-card {
  box-shadow: none;
}

.md-button {
  min-width: 64px;
}

.md-tooltip {
  background-color: #888 !important;
  color: #fff !important;
  border-radius: 3px;
  opacity: 1 !important;
}

.md-tooltip.md-origin-bottom {
  margin-top: -6px;
  margin-left: -8px;
}

.node-red-ui--notabs {
  color: #888;
  font-family: 'Helvetica Neue', Arial, Helvetica, sans-serif;
  display: flex;
  height: 100%;
  width: 100%;
  align-items: center;
  justify-content: center;
  opacity: 1;
  animation-name: fadeInOpacity;
  animation-iteration-count: 1;
  animation-timing-function: ease-in;
  animation-duration: 2.5s;
}

.node-red-ui--loading {
  color: #888;
  font-family: 'Helvetica Neue', Arial, Helvetica, sans-serif;
  display: flex;
  height: 100%;
  width: 100%;
  align-items: center;
  justify-content: center;
}

.node-red-ui--inline-link {
  width: 100%;
  height: 100%;
  position: absolute;
}

.node-red-ui--inline-link iframe {
  width: 100%;
  height: 100%;
}

.material-icons {
  vertical-align: middle;
}

.ie9 img[src$=".svg"] {
  width: 100%;
}

@media screen and (-ms-high-contrast: active),
(-ms-high-contrast: none) {
  img[src$=".svg"] {
    width: 100%;
  }
}

@keyframes fadeInOpacity {
  0% {
    opacity: 0;
  }

  30% {
    opacity: 0;
  }

  100% {
    opacity: 1;
  }
}

.flip-icon {
  animation: flip-icon 0.3s linear both;
}

@keyframes flip-icon {
  0% {
    transform: rotateY(180deg);
  }

  100% {
    transform: rotateY(0deg);
  }
}

@supports (-ms-overflow-style: none) {
  overflow: auto !important;
}

/*
    ALL COMPONENTS/CONTENT
*/
:root {
  --nr-dashboard-pageBackgroundColor: ${globalDashboardNode.theme.themeState['page-backgroundColor'].value};
  --nr-dashboard-pageTitlebarBackgroundColor: ${globalDashboardNode.theme.themeState['page-titlebar-backgroundColor'].value};
  --nr-dashboard-pageSidebarBackgroundColor: ${globalDashboardNode.theme.themeState['page-sidebar-backgroundColor'].value};
  --nr-dashboard-groupTextColor: ${globalDashboardNode.theme.themeState['group-textColor'].value};
  --nr-dashboard-groupBackgroundColor: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  --nr-dashboard-groupBorderColor: ${globalDashboardNode.theme.themeState['group-borderColor'].value};
  --nr-dashboard-widgetTextColor: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  --nr-dashboard-widgetBackgroundColor: ${globalDashboardNode.theme.themeState['base-color'].value};
  --nr-dashboard-widgetBorderColor: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  --nr-dashboard-widgetColor: ${globalDashboardNode.theme.themeState['base-color'].value};
  --nr-dashboard-widgetBgndColor: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
}

.nr-dashboard-disabled {
  opacity: 0.4 !important;
  pointer-events: none !important;
}

div.nr-dashboard-theme {
  background-color: ${globalDashboardNode.theme.themeState['page-backgroundColor'].value};
  font-family: -apple-system, BlinkMacSystemFont, Segoe UI, Roboto, Oxygen-Sans, Ubuntu, Cantarell, Helvetica Neue, sans-serif;
}

div.nr-dashboard-theme md-content {
  background: transparent;
  height: 100%;
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

div.nr-dashboard-theme md-input-container.md-input-focused:not([md-no-float]) .md-select-placeholder span:first-child {
  -webkit-transform: translateY(-28px) scale(0.75);
  transform: translateY(-28px) scale(0.75);
}

div.nr-dashboard-theme md-input-container.md-input-focused:not(.md-input-has-value) md-select .md-select-value.md-select-placeholder {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

div.nr-dashboard-theme md-content md-card {
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

/* Some colour classes */
div.nr-dashboard-theme .nr-dashboard-color {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

div.nr-dashboard-theme .nr-dashboard-error {
  color: #ff2929;
}

div.nr-dashboard-theme .nr-dashboard-warning {
  color: #eaf42f;
}

div.nr-dashboard-theme .nr-dashboard-ok {
  color: #49db3c;
}

div.nr-dashboard-theme .nr-dashboard-dim {
  opacity: 0.5 !important;
}

/*
    DASHBOARD HEADER
*/
div.nr-dashboard-theme md-toolbar {
  background-color: ${globalDashboardNode.theme.themeState['page-titlebar-backgroundColor'].value};
  color: #fff;
}

div.nr-dashboard-theme md-toolbar .md-toolbar-tools {
  color: #fff;
  filter: invert(0%);
}

/*
    DASHBOARD SIDEBAR
*/
div.nr-dashboard-theme md-sidenav {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
  background-color: ${globalDashboardNode.theme.themeState['page-sidebar-backgroundColor'].value};
}

div.nr-dashboard-theme md-sidenav md-list-item._md-button-wrap>div.md-button:first-child {
  background-color: transparent;
}

div.nr-dashboard-theme md-sidenav div.md-list-item-inner {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
  background-color: transparent;
}

a.md-no-style,
button.md-no-style {
  background-color: transparent;
}

.nr-menu-item-active div button {
  border-right: 4px solid ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

/*
    GROUP SPECIFIC
*/
.nr-dashboard-theme ui-card-panel {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  border: 1px solid ${globalDashboardNode.theme.themeState['group-borderColor'].value};
}

.nr-dashboard-theme ui-card-panel p.nr-dashboard-cardtitle {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

/*
    COMPONENT-SPECIFIC
*/
.nr-dashboard-theme .nr-dashboard-spacer {
  opacity: 0;
}

.md-button.md-default-theme.md-primary,
.md-button.md-primary {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
  color: white;
}

.nr-dashboard-theme .md-button[disabled] {
  color: rgba(0, 0, 0, 0.38);
  cursor: default;
}

/* Button */
.nr-dashboard-theme .nr-dashboard-button .md-button {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  color: white;
}

.nr-dashboard-theme .nr-dashboard-button .md-button:hover {
  background-color: #009f00;
}

.nr-dashboard-theme .nr-dashboard-template .md-button[disabled] {
  color: #888;
}

.nr-dashboard-theme .nr-dashboard-button .md-button.md-primary {
  color: white;
  background-color: #009400;
}

.nr-dashboard-theme .nr-dashboard-button .md-button.md-raised:not([disabled]).md-focused {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

/* Date picker */
.nr-dashboard-theme .nr-dashboard-date-picker md-input-container .md-input {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  border-bottom-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  font-family: inherit;
}

.nr-dashboard-theme .nr-dashboard-date-picker md-input-container .md-input::-webkit-input-placeholder {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-date-picker .md-button.md-icon-button {
  /* background-color: @widgetBackgroundColor; */
}

.nr-dashboard-theme .nr-dashboard-date-picker .md-button.md-icon-button md-icon {
  color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme .nr-dashboard-date-picker .md-datepicker-triangle-button .md-datepicker-expand-triangle {
  border-top-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.md-default-theme .md-calendar-day-header,
.md-calendar-day-header {
  color: #ffffff;
  background-color: ${globalDashboardNode.theme.themeState['page-titlebar-backgroundColor'].value};
}

.md-default-theme .md-calendar-date.md-calendar-selected-date .md-calendar-date-selection-indicator,
.md-calendar-date.md-calendar-selected-date .md-calendar-date-selection-indicator,
.md-default-theme .md-calendar-date.md-focus.md-calendar-selected-date .md-calendar-date-selection-indicator,
.md-calendar-date.md-focus.md-calendar-selected-date .md-calendar-date-selection-indicator {
  background: ${globalDashboardNode.theme.themeState['base-color'].value};
  color: #ffffff;
  border-color: transparent;
}

.md-default-theme .md-calendar-date.md-calendar-date-today .md-calendar-date-selection-indicator,
.md-calendar-date.md-calendar-date-today .md-calendar-date-selection-indicator {
  border: 1px solid ${globalDashboardNode.theme.themeState['base-color'].value};
}

.md-datepicker-calendar-pane.md-pane-open {
  width: 345px;
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
}

/* Chart */
.nr-dashboard-theme .nr-dashboard-chart-titlel {
  font-size: larger;
  font-weight: 500;
}

/* Colour Picker */
.nr-dashboard-theme .color-picker-panel {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  border-color: ${globalDashboardNode.theme.themeState['group-borderColor'].value};
  box-shadow: 0 0 1px rgba(255, 255, 255, 0.5);
}

.nr-dashboard-theme .color-picker-input-wrapper>input {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  font-family: inherit;
}

.nr-dashboard-theme .color-picker-wrapper .color-picker-panel.color-picker-panel-round .color-picker-grid-wrapper .color-picker-row .color-picker-grid .color-picker-grid-inner {
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
}

/* Dropdown */
.nr-dashboard-theme .nr-dashboard-dropdown md-select .md-select-value,
.nr-dashboard-theme .nr-dashboard-dropdown md-select .md-select-value.md-select-placeholder {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  border-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  border-bottom-width: 1px;
  padding: 0px 2px;
}

.nr-dashboard-theme .nr-dashboard-dropdown .md-select-icon {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .md-select-menu-container {
  max-height: 85%;
  overflow-y: auto;
  border: 1px solid ${globalDashboardNode.theme.themeState['page-titlebar-backgroundColor'].value};
}

.nr-dashboard-theme md-select-menu {
  margin-left: -2px;
  max-height: 85%;
}

.nr-dashboard-theme md-select-menu md-content {
  padding: 0px;
  max-height: 85%;
}

.nr-dashboard-theme md-select-menu,
.nr-dashboard-theme md-select-menu md-option {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme md-select-menu md-option[selected] {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value} !important;
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value} !important;
}

.nr-dashboard-theme md-select-menu[multiple] md-option.md-checkbox-enabled[selected] .md-icon {
  background-color: #00b300 !important;
  border-color: transparent !important;
}

.nr-dashboard-theme md-select-menu[multiple] md-option.md-checkbox-enabled .md-icon {
  border-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

.nr-dashboard-theme md-select-menu.md-default-theme md-option:focus:not([disabled]):not([selected]),
md-select-menu md-option:focus:not([disabled]):not([selected]) {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
}

.nr-dashboard-theme md-select-menu md-option:hover {
  background-color: #009400 !important;
}

.nr-dashboard-theme md-menu-content {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme md-menu-content md-menu-item {
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

/* Gauge */
.nr-dashboard-theme .nr-dashboard-gauge text {
  fill: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  margin: 8px;
}

.nr-dashboard-theme .nr-dashboard-gauge-title {
  margin: 6px 0px 20px 0px;
  z-index: 1;
}

.nr-dashboard-theme .nr-dashboard-gauge-titlel {
  font-size: larger;
  font-weight: 500;
  z-index: 1;
}

.nr-dashboard-theme .nr-dashboard-gauge-titlem {
  font-weight: 500;
  margin: 2px 0px 14px 0px;
  z-index: 1;
}

.nr-dashboard-theme .nr-dashboard-gauge-titles {
  font-size: x-small;
  margin-top: 2px;
  height: 10px;
  padding-bottom: 14px;
  z-index: 1;
}

/* Numeric */
.nr-dashboard-theme .nr-dashboard-numeric .value {
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

/* Slider */
.nr-dashboard-theme .nr-dashboard-slider .md-track {
  background-color: rgba(111, 111, 111, 0.5);
}

.nr-dashboard-theme .nr-dashboard-slider .md-track-fill {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme .nr-dashboard-slider .md-thumb:after {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  border-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme .nr-dashboard-slider .md-focus-ring {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  transform: scale(0.4);
  opacity: 1;
}

.nr-dashboard-theme .nr-dashboard-slider .md-sign {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme .nr-dashboard-slider .md-sign:after {
  border-top-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

.nr-dashboard-theme .nr-dashboard-slider .md-min[md-discrete] .md-sign {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme .nr-dashboard-slider md-slider[md-discrete] .md-sign,
.nr-dashboard-theme .nr-dashboard-slider md-slider[md-discrete] .md-sign:after {
  opacity: 1;
  -webkit-transform: translate3d(0, 0, 0) scale(1);
  transform: translate3d(0, 0, 0) scale(1);
}

.nr-dashboard-theme .nr-dashboard-slider .md-sign:after {
  border-top-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme md-slider.md-default-theme[md-vertical] .md-sign:after,
.nr-dashboard-theme md-slider[md-vertical] .md-sign:after {
  content: none;
}

.nr-dashboard-theme .nr-dashboard-slider md-slider.md-default-theme.md-min[md-discrete] .md-thumb:after,
md-slider.md-min[md-discrete] .md-thumb:after {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  border-color: transparent;
}

.nr-dashboard-theme .nr-dashboard-slider md-slider .md-track-ticks {
  color: transparent;
}

.nr-dashboard-theme .nr-dashboard-slider md-slider[md-discrete]:not([disabled]) .md-slider-wrapper.md-focused .md-focus-ring {
  transform: scale(0.4);
  animation: unset;
}

.nr-dashboard-theme .nr-dashboard-slider .md-thumb-text {
  color: white;
  filter: invert(0%);
}

/* Switch */
.nr-dashboard-theme .nr-dashboard-switch md-switch .md-bar {
  background-color: rgba(111, 111, 111, 0.5);
}

.nr-dashboard-theme .nr-dashboard-switch md-switch.md-checked .md-bar {
  background-color: rgba(0, 128, 0, 0.5);
}

.nr-dashboard-theme .nr-dashboard-switch md-switch .md-thumb {
  background-color: #949494;
}

.nr-dashboard-theme .nr-dashboard-switch md-switch.md-checked .md-thumb {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

/* Template */
md-list {
  padding: unset;
}

md-list-item[disabled] {
  color: #888;
}

md-list-item {
  min-height: 48px;
  height: 48px;
}

md-list-item._md-button-wrap>div.md-button:first-child {
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  height: inherit;
  padding: 0 6px;
}

md-list-item.md-2-line {
  height: 48px;
  min-height: 48px;
}

md-list-item.md-3-line {
  height: 72px;
  min-height: 72px;
}

md-list-item .md-list-item-inner {
  background: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
  padding-left: 8px;
  width: 100%;
  height: 100%;
}

md-progress-linear {
  padding-top: 4px !important;
}

md-progress-linear .md-container {
  margin-top: 12px;
  background-color: ${globalDashboardNode.theme.themeState['page-backgroundColor'].value};
}

md-progress-linear .md-container .md-bar {
  background-color: #009a00;
}

md-checkbox {
  min-height: 48px;
  margin-left: 6px;
  margin-bottom: 0px;
  width: 100%;
}

md-checkbox .md-icon {
  border-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

md-checkbox[disabled] .md-icon {
  border-color: #888 !important;
}

md-checkbox.md-checked .md-icon {
  background-color: #009a00;
  border-color: transparent !important;
}

md-checkbox .md-label {
  position: absolute;
  top: 32%;
}

md-checkbox[disabled] .md-label {
  color: #888;
}

md-checkbox:last-of-type {
  margin-left: 6px;
}

.nr-dashboard-theme .nr-dashboard-template {
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value};
}

.nr-dashboard-theme .nr-dashboard-template md-switch .md-thumb {
  background-color: #949494;
}

.nr-dashboard-theme .nr-dashboard-template md-switch .md-bar {
  background-color: rgba(111, 111, 111, 0.5);
}

.nr-dashboard-theme .nr-dashboard-template md-switch.md-checked:not([disabled]) .md-thumb {
  background-color: #009a00;
}

.nr-dashboard-theme .nr-dashboard-template md-switch.md-checked:not([disabled]) .md-bar {
  background-color: rgba(0, 128, 0, 0.5);
}

md-radio-button {
  min-height: 48px;
  margin-left: 6px;
  margin-bottom: 0px;
}

md-radio-button .md-off,
md-radio-button .md-on {
  border-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

md-radio-button[disabled] .md-off,
md-radio-button[disabled] .md-on {
  border-color: #888 !important;
}

md-radio-button.md-checked .md-on {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

md-radio-button .md-label {
  position: absolute;
  top: 32%;
}

md-radio-button[disabled] .md-label {
  color: #888;
}

md-divider {
  border-color: ${globalDashboardNode.theme.themeState['base-color'].value} !important;
}

.nr-dashboard-theme .nr-dashboard-template path {
  fill: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-template {
  padding: 3px 6px;
}

.nr-dashboard-template p,
.nr-dashboard-template h1,
.nr-dashboard-template h2,
.nr-dashboard-template h3,
.nr-dashboard-template h4 {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value} !important;
  background-color: ${globalDashboardNode.theme.themeState['widget-borderColor'].value} !important;
}

.nr-dashboard-theme .nr-dashboard-template .md-button .md-no-style {
  background: initial !important;
  color: initial !important;
}

.nr-dashboard-theme .nr-dashboard-template .md-button[disabled] {
  color: #ccc;
}

.nr-dashboard-theme .nr-dashboard-template .md-button.md-primary:not([disabled]) {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  background-color: #009a00;
}

.nr-dashboard-theme .nr-dashboard-template .md-button .ng-scope {
  color: unset;
}

.nr-dashboard-template .md-button {
  color: white;
}

.nr-dashboard-theme .nr-dashboard-template .md-button:not(:first-of-type) {
  margin-top: 18px;
}

.nr-dashboard-theme .nr-dashboard-template .md-button {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  margin: 0;
  min-height: 36px;
  min-width: unset;
  line-height: unset;
}

.nr-dashboard-theme .nr-dashboard-template .md-button.md-fab.md-mini {
  padding: unset;
}

.nr-dashboard-theme .nr-dashboard-template .md-button:not([disabled]):hover {
  background-color: #009400;
}

.nr-dashboard-theme .nr-dashboard-template .md-button.md-primary:not([disabled]):hover {
  background-color: #009400;
}

.nr-dashboard-theme .nr-dashboard-template .md-button.md-primary.md-raised:not([disabled]) {
  background-color: #008a00;
}

.nr-dashboard-theme .nr-dashboard-template .md-button.md-primary.md-raised:not([disabled]):hover {
  background-color: #009400;
}

.nr-dashboard-theme .nr-dashboard-template ::-webkit-scrollbar {
  height: 12px;
  width: 12px;
}

.nr-dashboard-theme .nr-dashboard-template ::-webkit-scrollbar-thumb {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  -webkit-border-radius: 1ex;
  -webkit-box-shadow: 0px 1px 2px rgba(0, 0, 0, 0.75);
}

.nr-dashboard-theme .nr-dashboard-template ::-webkit-scrollbar-track {
  background-color: ${globalDashboardNode.theme.themeState['page-backgroundColor'].value};
}

/* Text-Input */
.nr-dashboard-theme .nr-dashboard-textinput label {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-textinput md-input-container.md-input-has-value label {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-textinput md-input-container.md-input-focused label {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-textinput md-input-container.md-input-focused input {
  border-color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
  font-family: inherit;
}

.nr-dashboard-theme .nr-dashboard-textinput md-input-container .md-input:invalid {
  color: #DA5252;
}

.nr-dashboard-theme .nr-dashboard-textinput input {
  border-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  font-family: inherit;
}

.nr-dashboard-theme input[type="time"]::-webkit-calendar-picker-indicator {
  filter: invert(100%);
}

.nr-dashboard-theme input[type="date"]::-webkit-calendar-picker-indicator {
  filter: invert(100%);
}

/* Text */
/* Dialog */
.nr-dashboard-theme md-dialog {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  background-color: ${globalDashboardNode.theme.themeState['group-backgroundColor'].value};
  border: 3px solid ${globalDashboardNode.theme.themeState['group-borderColor'].value};
}

.nr-dashboard-theme .md-dialog-content {
  padding: 24px;
}

.nr-dashboard-theme md-input-container.md-default-theme .md-input,
md-input-container .md-input {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  border-color: ${globalDashboardNode.theme.themeState['page-titlebar-backgroundColor'].value} !important;
}

/* Form */
.nr-dashboard-theme .nr-dashboard-form md-input-container {
  display: block;
}

.nr-dashboard-theme .nr-dashboard-form-button {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
  color: white;
}

.nr-dashboard-theme .nr-dashboard-form-button:hover {
  background-color: #00b300 !important;
}

.nr-dashboard-theme .nr-dashboard-form-button:focus {
  background-color: #00b300 !important;
}

.nr-dashboard-theme .nr-dashboard-form {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-form form md-input-container input {
  border-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
  font-family: inherit;
}

.nr-dashboard-theme .nr-dashboard-form form md-input-container label {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-form form md-input-container.md-input-has-value:not(.md-input-invalid) label {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-form form md-input-container:not(.md-input-invalid).md-input-focused input {
  border-color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-form form md-input-container:not(.md-input-invalid).md-input-focused label {
  color: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

.nr-dashboard-theme .nr-dashboard-form form .md-input-invalid input {
  border-color: #d00 !important;
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-input-invalid label {
  color: #d00 !important;
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-checkbox.md-checked .md-icon {
  background-color: #00b300;
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-checkbox .md-icon {
  border-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-switch .md-bar {
  background-color: rgba(111, 111, 111, 0.5);
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-switch .md-thumb {
  background-color: #949494;
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-switch.md-checked .md-bar {
  background-color: #00b300;
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container md-switch.md-checked .md-thumb {
  background-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-datepicker {
  background-color: inherit;
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-datepicker-calendar-icon {
  fill: ${globalDashboardNode.theme.themeState['group-textColor'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-datepicker-open .md-datepicker-calendar-icon {
  fill: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-datepicker-input {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-datepicker-input-container {
  border-color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form .md-datepicker-expand-triangle {
  border-top-color: ${globalDashboardNode.theme.themeState['base-color'].value};
}

.nr-dashboard-theme md-content .nr-dashboard-form form md-input-container .md-input {
  color: ${globalDashboardNode.theme.themeState['widget-textColor'].value};
}
<style>
`
$('head').append(myTheme)